...
* 商人和护士改成首次入住后就算解锁，后面被打死只要有空房就能正常重生
* 新增腐化草地和猩红草地，世界会在第一次运行时 50/50 选一种作为当前世界主邪恶
* 邪恶草地生成和蔓延改成世界级固定预算更新，只在已加载区域抽样扩散，不走每块方块乱 random tick
* 先用原版贴图占位把方块资源、掉落和物品模型接通，后面你直接替换贴图就行
* 猩红和腐化现在改成各自按源块蔓延，世界只限制“自然生成哪一种”，不再限制手动放出来的另一种不扩散
* 商人和护士的到达刷点改成和向导同一套地表空气位查找，避免直接拿玩家脚下方块位导致刷不出来
* 邪恶蔓延速度恢复到正常固定预算，调试提速常量还是保留在 `WorldEvilSystem` 里方便以后再改
* 新增 `/hushi` 和 `/shangran`（也兼容 `/shangren`）诊断命令，直接输出空房、首次生成/重生状态和缺少的条件
* 修正 Town NPC 卡在 `spawnedOnce=true` 但实体已丢失时不会回来的状态，现在会按重生继续调度
* 收紧 Town NPC 的丢实体重生兜底，避免把向导也走进这条分支导致重复刷两个向导
